...
//import com.LOGamez.explodes.CreditsExplode;
...
public class CreditsMenu extends Menu {
    ...
    /**Creditz1 List of CreditsMenu*/
    public static List<Rectangle> Creditz1 = new ArrayList<>();
    
    /**CreditsRect List of CreditsMenu*/
    ...
    */
    public CreditsMenu(Game game) {
        ...
        this.game = game;
        //target = new Render(BlockBreaker.getMainWidth(), BlockBreaker.getMainHeight());
    ...
    public void render(Render target, Graphics2D g) {
        ...
        if(CMTimer < 450 && CMTimer > 0){
            if(System.currentTimeMillis() % 47 == 0){
                ...
                target.scaleDraw(Texture.SI2015Logo, 1, MazeGenerator.getMainWidth()-40-(CMTimer/2), 288, 0, 0, 281, 271, 0x000000);
                target.scaleDraw(Texture.SI2015Logo, 1, MazeGenerator.getMainWidth()-38-(CMTimer/2), 286, 0, 0, 281, 271, 0xffffff);
        ...
        if(CMTimer == 0){
            ...
            if(count % 18 == 0){
                int x = MazeGenerator.getMainWidth()/2 - 168+count;
                double dx =  x * Math.sin(count/6);
                int y = MazeGenerator.getMainHeight()/2 - 108-count;
                ...
                animCredits.render(target, MazeGenerator.getMainWidth()/2 - 168-count, MazeGenerator.getMainHeight()/2 - 108-count, 1, 340, 180, creditsColor);
                animCredits.render(target, MazeGenerator.getMainWidth()/2 - 170-count, MazeGenerator.getMainHeight()/2 - 110-count, 1, 340, 180, 0xffffff);

                animCreditsA.render(target, MazeGenerator.getMainWidth()/2 - 168-count, MazeGenerator.getMainHeight()/2 - 108+count, 1, 340, 180, creditsColor);
                animCreditsA.render(target, MazeGenerator.getMainWidth()/2 - 170-count, MazeGenerator.getMainHeight()/2 - 110+count, 1, 340, 180, 0xffffff);

                animCreditsA.render(target, MazeGenerator.getMainWidth()/2 - 168+count, MazeGenerator.getMainHeight()/2 - 108+count, 1, 340, 180, creditsColor);
                animCreditsA.render(target, MazeGenerator.getMainWidth()/2 - 170+count, MazeGenerator.getMainHeight()/2 - 110+count, 1, 340, 180, 0xffffff);
            }
            
            animCreditsB.render(target, MazeGenerator.getMainWidth()/2 - 168, MazeGenerator.getMainHeight()/2 - 108, 1, 340, 180, creditsColor);
            animCreditsB.render(target, MazeGenerator.getMainWidth()/2 - 170, MazeGenerator.getMainHeight()/2 - 110, 1, 340, 180, 0xffffff);
        ...
        if(CMSkipTimer < 20){
            ...
            if(MouseInput.MouseX > 500 && MouseInput.MouseX < 500 + 80 && MouseInput.MouseY > 630 && MouseInput.MouseY < 630 + 30) {
                ...
                //Set Status Bar: Skip
                MazeGenerator.setStatusBar("Skip");
                ...
                MazeGenerator.setStatusBar("Credits Menu");
            }
        }
        ...
        //target.draw(target, 0, 0);
        //g.setColor(Color.BLACK);
        //g.fillRect(0, 0, BlockBreaker.getMainWidth()+12, BlockBreaker.getMainHeight());
        
//        for (int i = 0; i < game.pixels.length; i++) {
//            game.pixels[i] = 0xffffff;
//        }
